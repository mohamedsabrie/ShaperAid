    ...
    <div className=" overflow-auto  scrollbar-hide">
      ...
      <tbody>
        {data.map((item, index) => {
          ...
          return (
            <tr key={item.id} className={`w-full ${index % 2 ? "" : "bg-[#ffffff13]"} `}>
              <td className="border-y border-white p-[10px] sm:p-3 desc ">{renderTd(item.row[0]) } </td>
              <td className="border-y border-white p-[10px] sm:p-3 desc ">{renderTd(item.row[1])} </td>
            </tr>
          );
        })}
      </tbody>
